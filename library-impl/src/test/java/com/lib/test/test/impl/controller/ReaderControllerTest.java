...
class ReaderControllerImplTest {
    ...
    @Test
    void testLogin() {
        ReaderAuthRequest req = new ReaderAuthRequest();
        req.setName("user");
        req.setPassword("pass");

        ReaderAuthResponse expectedResponse = new ReaderAuthResponse("token");

        when(authService.authenticate(req)).thenReturn(expectedResponse);

        ReaderAuthResponse result = controller.login(req);
